...
void scheduler(){
    // No explicit selection needed since the running process is implicit
    // In a real system, this would perform a context switch
}

int create(int p){
    int parent = getRunningProcess();
    if (parent == -1){ // Fixed comparison (was using assignment)
        return -1;
    }
    ...
    for (i = 0; i < 16; i++){
        if (pcb[i].state == FREE){
            child = i; // Fixed assignment (was setting to 1)
            break;
        }
    }

    if (child == -1){ // Fixed condition (was checking for 1)
        return -1;
    }
...
int main(int argc, char *argv[]) {
    ...
    while (fgets(command, sizeof(command), input_file)){
        ...
        if (newline){
            ...
            fprintf(output_file, " %d", result); // Added space here!
        }
    }
